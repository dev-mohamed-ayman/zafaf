@extends('admin.layouts.master')
@section('content')
    <div class="card">
        <div class="card-header">
            <h3 class="float-left">المدن</h3>
            @can('city create')
                <button type="button" id="toast" class="btn btn-primary float-right" data-toggle="modal"
                        data-target="#create">اضافة
                    مدينه
                </button>
            @endcan
        </div>
        <div class="card-body">
            <div class="table-responsive">
                <table class="table table-striped table-hover table-bordered text-center">
                    <thead>
                    <tr>
                        <th>#</th>
                        <th>الدولة</th>
                        <th>اسم المدينة بالعربي</th>
                        <th>اسم المدينة بالانجليزي</th>
                        <th>العمليات</th>
                    </tr>
                    </thead>
                    <tbody>
                    @forelse($items as $item)
                        <tr>
                            <th>{{$loop->index + 1}}</th>
                            <td>
                                @if($item->country == 'EG')
                                    مصر
                                @elseif($item->country == 'SA')
                                    السعودية
                                @elseif($item->country == 'AE')
                                    الامارات
                                @endif
                            </td>
                            <td>{{$item->title_ar}}</td>
                            <td>{{$item->title_en}}</td>
                            <td>
                                @can('city update')
                                    <button type="button" id="toast" class="btn btn-success" data-toggle="modal"
                                            data-target="#update{{$item->id}}">
                                        <i class="fa fa-edit"></i>
                                    </button>
                                @endcan
                                @can('city delete')
                                    <button type="button" id="toast" class="btn btn-danger" data-toggle="modal"
                                            data-target="#delete{{$item->id}}">
                                        <i class="fa fa-trash"></i>
                                    </button>
                                @endcan
                            </td>
                        </tr>
                        {{--Modal Update--}}
                        <div class="modal fade" id="update{{$item->id}}">
                            <div class="modal-dialog">
                                <div class="modal-content">
                                    <div class="modal-header">
                                        <h4 class="modal-title">تعديل</h4>
                                        <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                                            <span aria-hidden="true">&times;</span>
                                        </button>
                                    </div>
                                    <form action="{{route('admin.city.update', $item->id)}}" method="post">
                                        @csrf
                                        @method('put')
                                        <div class="modal-body">
                                            <div class="form-group">
                                                <label for="country" class="col-form-label">الدولة</label>
                                                <select name="country" id="country" class="form-control">
                                                    <option selected disabled>اختر دولة</option>
                                                    <option value="EG" {{$item->country == 'EG' ? 'selected' : ''}}>
                                                        مصر
                                                    </option>
                                                    <option value="SA" {{$item->country == 'SA' ? 'selected' : ''}}>
                                                        السعودية
                                                    </option>
                                                    <option value="AE" {{$item->country == 'AE' ? 'selected' : ''}}>
                                                        الامارات
                                                    </option>
                                                </select>
                                            </div>
                                            <div class="form-group">
                                                <label for="title_ar" class="col-form-label">اسم المدينة بالعربي</label>
                                                <input type="text" name="title_ar" id="title_ar"
                                                       value="{{old('title_ar', $item->title_ar)}}" class="form-control"
                                                       placeholder="اسم المدينة بالعربي">
                                            </div>
                                            <div class="form-group">
                                                <label for="title_en" class="col-form-label">اسم المدينة
                                                    بالانجليزي</label>
                                                <input type="text" name="title_en" id="title_en"
                                                       value="{{old('title_en', $item->title_en)}}" class="form-control"
                                                       placeholder="اسم المدينة بالانجليزي">
                                            </div>
                                        </div>
                                        <div class="modal-footer justify-content-between">
                                            <button type="submit" class="btn btn-primary">حفظ</button>
                                            <button type="button" class="btn btn-secondary" data-dismiss="modal">اغلاق
                                            </button>
                                        </div>
                                    </form>
                                </div>
                                <!-- /.modal-content -->
                            </div>
                            <!-- /.modal-dialog -->
                        </div>
                        {{--Modal Delete--}}
                        <div class="modal fade" id="delete{{$item->id}}">
                            <div class="modal-dialog">
                                <div class="modal-content">
                                    <div class="modal-header">
                                        <h4 class="modal-title">حذف</h4>
                                        <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                                            <span aria-hidden="true">&times;</span>
                                        </button>
                                    </div>
                                    <form action="{{route('admin.city.destroy', $item->id)}}" method="post">
                                        @csrf
                                        @method('delete')
                                        <div class="modal-body">
                                            <div class="bg-danger p-3 text-center">
                                                هل ان متاكد من الحذف
                                            </div>
                                        </div>
                                        <div class="modal-footer justify-content-between">
                                            <button type="submit" class="btn btn-primary">نعم</button>
                                            <button type="button" class="btn btn-secondary" data-dismiss="modal">اغلاق
                                            </button>
                                        </div>
                                    </form>
                                </div>
                                <!-- /.modal-content -->
                            </div>
                            <!-- /.modal-dialog -->
                        </div>
                    @empty
                        <tr>
                            <td colspan="1000">لا يوجد بيانات</td>
                        </tr>
                    @endforelse
                    </tbody>
                </table>
                <div>
                    {{$items->links()}}
                </div>
            </div>
        </div>
    </div>
    {{--Modal Create--}}
    <div class="modal fade" id="create">
        <div class="modal-dialog">
            <div class="modal-content">
                <div class="modal-header">
                    <h4 class="modal-title">اضافة مدينة</h4>
                    <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                        <span aria-hidden="true">&times;</span>
                    </button>
                </div>
                <form action="{{route('admin.city.store')}}" method="post">
                    @csrf
                    <div class="modal-body">
                        <div class="form-group">
                            <label for="country" class="col-form-label">الدولة</label>
                            <select name="country" id="country" class="form-control">
                                <option selected disabled>اختر دولة</option>
                                <option value="EG">مصر</option>
                                <option value="SA">السعودية</option>
                                <option value="AE">الامارات</option>
                            </select>
                        </div>
                        <div class="form-group">
                            <label for="title_ar" class="col-form-label">اسم المدينة بالعربي</label>
                            <input type="text" name="title_ar" id="title_ar" value="{{old('title_ar')}}"
                                   class="form-control"
                                   placeholder="اسم المدينة بالعربي">
                        </div>
                        <div class="form-group">
                            <label for="title_en" class="col-form-label">اسم المدينة بالانجليزي</label>
                            <input type="text" name="title_en" id="title_en" value="{{old('title_en')}}"
                                   class="form-control"
                                   placeholder="اسم المدينة بالانجليزي">
                        </div>
                    </div>
                    <div class="modal-footer justify-content-between">
                        <button type="submit" class="btn btn-primary">حفظ</button>
                        <button type="button" class="btn btn-secondary" data-dismiss="modal">اغلاق</button>
                    </div>
                </form>
            </div>
            <!-- /.modal-content -->
        </div>
        <!-- /.modal-dialog -->
    </div>
@endsection
